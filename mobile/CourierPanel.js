...
const UPLOAD_URL = `${API_BASE_URL}/api/v1/Cargo/upload`;
...
export default function CourierPanel({ onBack, token }) {
  ...
  const [upCargoRef, setUpCargoRef] = useState('');
  ...
  const takePicture = async () => {
    ...
    try {
      ...
      setUpPhoto(taken);
      setCameraMode(false);
      Alert.alert('✅ Fotoğraf Hazır', 'Görsel kaydedildi.');
    } catch {
  ...
  const handleUpload = async () => {
    if (!upPhoto) { Alert.alert('⚠️', 'Lütfen önce kutu fotoğrafını çekin.'); return; }
    setUpLoading(true);
    try {
      const formData = new FormData();
      formData.append('file', { uri: upPhoto.uri, name: `kargo_${upCargoRef || 'x'}_${Date.now()}.jpg`, type: 'image/jpeg' });
      formData.append('sarsinti_verisi', maxGForce.toFixed(2));
      formData.append('is_fragile', isFragile.toString());
      if (upCargoRef.trim()) formData.append('cargo_ref_id', upCargoRef.trim());
      ...
      if (res.ok && ct.includes('application/json')) {
        ...
        Alert.alert('🚀 Gönderildi!',
          `Analiz kuyruğuna alındı.\n📦 Takip: ${upCargoRef || 'Belirtilmedi'}\n💥 Maks: ${maxGForce.toFixed(2)}G${isFragile ? '\n🍷 Hassas — IoT aktif' : ''}`);
        setUpPhoto(null); setUpCargoRef(''); setIsFragile(false); setMaxGForce(1.0);
      } else {
        Alert.alert('Hata', `HTTP ${res.status}`);
      }
    } catch {
    ...
    <SafeAreaView style={s.safe}>
      ...
      <ScrollView style={s.scroll} contentContainerStyle={s.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.card}>
          ...
          <TextInput style={s.input} placeholder="Takip No" placeholderTextColor="#94a3b8"
            value={upCargoRef} onChangeText={setUpCargoRef} returnKeyType="done" />
        </View>
        ...
        <View style={s.card}>
          ...
          {upPhoto ? (
            <View style={s.photoRow}>
              ...
              <TouchableOpacity onPress={() => setUpPhoto(null)}>
        ...
        <TouchableOpacity style={[s.btnSend, upLoading && s.btnSendDisabled]}
          onPress={handleUpload} disabled={upLoading} activeOpacity={0.85}>
          {upLoading ? <ActivityIndicator size="small" color="#fff" /> : <Text style={s.btnSendText}>🚀 Analizi Başlat</Text>}
        </TouchableOpacity>
...
const s = StyleSheet.create({
  ...
  input: {
    ...
    color: 'white', fontSize: 18, paddingHorizontal: 16, paddingVertical: 14, fontWeight: '600',
  },
  ...
  btnCameraSuccess: { borderColor: '#2563eb', backgroundColor: '#0d1f3c' },
  btnCameraText:    { color: 'white', fontSize: 18, fontWeight: '800' },

  btnSend: {
    backgroundColor: '#2563eb', borderRadius: 16, paddingVertical: 22, alignItems: 'center',
    elevation: 6, shadowColor: '#2563eb', shadowOpacity: 0.4, shadowOffset: { width: 0, height: 4 }, shadowRadius: 10,
  },
